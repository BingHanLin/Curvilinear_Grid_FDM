...
class OperatorFDM3D:
    ...
    def __init__(self, Mesh):
        self._Mesh = Mesh
        self._axes = ('i','j','k')
        self._node_num = dict( zip( self._axes, Mesh.mesh_size ) )
        self._matrix_len = self._node_num['i']*self._node_num['j']*self._node_num['k']
        
        if self._Mesh.dim == 1:
            assert self._node_num['i'] >=4
        elif self._Mesh.dim == 2:
            assert ( self._node_num['i'] >=4 and
                     self._node_num['j'] >=4)
        elif self._Mesh.dim == 3:
            assert ( self._node_num['i'] >=4 and 
                     self._node_num['j'] >=4 and 
                     self._node_num['k'] >=4   )
...
# http://olsthoorn.readthedocs.io/en/latest/03_fdm_as_python_func.html
    ...
    def der_1(self, axis='i'):
        ...
        Ii, Ij, Ik = sp.eye(self._node_num['i']), sp.eye(self._node_num['j']) , sp.eye(self._node_num['k'])

        di_1D = sp.diags([-0.5, 0.5], [-1, 1], shape=(self._node_num['i'], self._node_num['i']))
        dj_1D = sp.diags([-0.5, 0.5], [-1, 1], shape=(self._node_num['j'], self._node_num['j']))
        dk_1D = sp.diags([-0.5, 0.5], [-1, 1], shape=(self._node_num['k'], self._node_num['k']))
        ...
        if axis == 'i':
            ...
            self.csr_zero_rows(der_1_3D, self._Mesh.get_node_index_list(i_front=True,i_end=True))
            self.csr_zero_rows(front_bound_matrix,  self._Mesh.get_node_index_list(i_front=True,inverse = True))
            self.csr_zero_rows(end_bound_matrix, self._Mesh.get_node_index_list(i_end=True,inverse = True))

        elif axis == 'j':
            der_1_3D = sp.csr_matrix( sp.kron(Ik, sp.kron(dj_1D, Ii)) )
            self.csr_zero_rows(der_1_3D, self._Mesh.get_node_index_list(j_front=True,j_end=True))
            self.csr_zero_rows(front_bound_matrix,  self._Mesh.get_node_index_list(j_front=True,inverse = True))
            self.csr_zero_rows(end_bound_matrix, self._Mesh.get_node_index_list(j_end=True,inverse = True))

        elif axis == 'k':
            der_1_3D = sp.csr_matrix( sp.kron(dk_1D, sp.kron(Ii, Ij)) )
            self.csr_zero_rows(der_1_3D, self._Mesh.get_node_index_list(k_front=True,k_end=True))
            self.csr_zero_rows(front_bound_matrix,  self._Mesh.get_node_index_list(k_front=True,inverse = True))
            self.csr_zero_rows(end_bound_matrix, self._Mesh.get_node_index_list(k_end=True,inverse = True))
    ...
    def der_2(self, axis='i'):
        ...
        dii_1D = sp.diags([1, -2, 1], [-1, 0, 1], shape=(self._node_num['i'], self._node_num['i']))
        djj_1D = sp.diags([1, -2, 1], [-1, 0, 1], shape=(self._node_num['j'], self._node_num['j']))
        dkk_1D = sp.diags([1, -2, 1], [-1, 0, 1], shape=(self._node_num['k'], self._node_num['k']))
        ...
        if axis == 'i':
            ...
            self.csr_zero_rows(der_2_3D, self._Mesh.get_node_index_list(i_front=True,i_end=True))
            self.csr_zero_rows(front_bound_matrix,  self._Mesh.get_node_index_list(i_front=True,inverse = True))
            self.csr_zero_rows(end_bound_matrix, self._Mesh.get_node_index_list(i_end=True,inverse = True))

        elif axis == 'j':
            der_2_3D = sp.csr_matrix( sp.kron(Ik, sp.kron(djj_1D, Ii)) )
            self.csr_zero_rows(der_2_3D, self._Mesh.get_node_index_list(j_front=True,j_end=True))
            self.csr_zero_rows(front_bound_matrix,  self._Mesh.get_node_index_list(j_front=True,inverse = True))
            self.csr_zero_rows(end_bound_matrix, self._Mesh.get_node_index_list(j_end=True,inverse = True))

        elif axis == 'k':
            der_2_3D = sp.csr_matrix( sp.kron(dkk_1D, sp.kron(Ii, Ij)) )
            self.csr_zero_rows(der_2_3D, self._Mesh.get_node_index_list(k_front=True,k_end=True))
            self.csr_zero_rows(front_bound_matrix,  self._Mesh.get_node_index_list(k_front=True,inverse = True))
            self.csr_zero_rows(end_bound_matrix, self._Mesh.get_node_index_list(k_end=True,inverse = True))
    ...
    def check_dim(self, axis):

        if self._Mesh.dim == 1:
            assert "j" not in axis
            assert "k" not in axis
        elif self._Mesh.dim == 2:
            assert "k" not in axis
